...
#define SCREEN_BUFFER_ADDRESS 0xd654
#define GLYPH_WIDTH 16	// 假设每个字形宽度为16像素
#define GLYPH_HEIGHT 16 // 假设每个字形高度为16像素
using uint32_t = unsigned long;
using uint16_t = unsigned short;
using uint8_t = unsigned char;
void draw_glyph(uint16_t chr, uint8_t x, uint8_t y)
{
	const auto width = 10;
	...
	if (n2 > width)
		n2 = width;
	const auto buf = (byte *)0xf800;
	const auto bs = (ushort *)normal_font + index2 * 12;
	for (int j = 0; j < height; j++)
	{
		ushort dat = (((bs[index2]) << n) | ((bs[index2 + height]) >> n2)) & 0b1111111111000000;
		ScreenSelect = 0;
		*(ushort *)(&buf[(j+1+y) * 0x20 + x]) = byteswap_16(dat);
		ScreenSelect = 4;
		*(ushort *)(&buf[(j+1+y) * 0x20 + x]) = byteswap_16(dat);
		index2++;
	}
